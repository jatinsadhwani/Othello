defmodule OthelloWeb.PlayerControllerTest do
  use OthelloWeb.ConnCase

  alias Othello.Games

  @create_attrs %{gamename: "some gamename", playername: "some playername"}
  @update_attrs %{gamename: "some updated gamename", playername: "some updated playername"}
  @invalid_attrs %{gamename: nil, playername: nil}
  ...
  describe "update player" do
    ...
    test "redirects when data is valid", %{conn: conn, player: player} do
      ...
      assert html_response(conn, 200) =~ "some updated gamename"
    end
